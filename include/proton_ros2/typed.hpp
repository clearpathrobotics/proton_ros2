...
struct TypedPublisher : public IPublisher
{
  ...
  {
    pub = node->create_publisher<rclcpp::TypeAdapter<proton::BundleHandle, MsgT>>(topic, qos);
  }

  void publish(const proton::BundleHandle & bundle) override { pub->publish(bundle); }

  std::string getTopic() override { return std::string(pub->get_topic_name()); }
...
template <typename MsgT>
struct TypedSubscriber : public ISubscriber
{
  TypedSubscriber(
    rclcpp::Node * node, const std::string & topic, const rclcpp::QoS & qos,
    proton::BundleHandle & bundle, proton::BundleHandle::BundleCallback callback)
  {
    sub = node->create_subscription<MsgT>(
      topic, qos,
      [&bundle, callback](const MsgT & msg)
      {
  ...
  std::string getTopic() override { return std::string(sub->get_topic_name()); }
...
struct IService
{
  ...
  void responseCallback(proton::BundleHandle & handle) { response_promise.set_value(handle); }
  ...
  bool waitForFuture(proton::BundleHandle & handle)
  {
    ...
    // Wait for future
    if (
      response_future.wait_for(std::chrono::milliseconds(timeout_ms_)) == std::future_status::ready)
    {
      // Get response
...
struct TypedService : public IService
{
  ...
  TypedService(
    ...
    const uint32_t timeout_ms, proton::BundleHandle & request_bundle,
    proton::BundleHandle & response_bundle, CallbackT callback)
  {
    timeout_ms_ = timeout_ms;

    srv = node->create_service<ServiceT>(
      service_name,
      [this, node, service_name, &request_bundle, &response_bundle, callback](
        const std::shared_ptr<RequestROS> request, std::shared_ptr<ResponseROS> response)
      {
        ...
        {
          RCLCPP_ERROR(
            node->get_logger(), "Service %s timed out waiting for a response",
            service_name.c_str());
        }
  ...
  TypedService(
    rclcpp::Node * node, const std::string & service_name, const rclcpp::QoS & qos,
    const uint32_t timeout_ms, proton::BundleHandle & request_bundle, CallbackT callback)
  {
    timeout_ms_ = timeout_ms;

    srv = node->create_service<ServiceT>(
      service_name,
      [this, &request_bundle, callback](
        const std::shared_ptr<RequestROS> request, std::shared_ptr<ResponseROS> response)
      {
        RCL_UNUSED(response);
        // Convert ROS request to Bundle
  ...
  std::string getServiceName() override { return std::string(srv->get_service_name()); }
...
struct TypedClient : public IClient
{
  ...
  TypedClient(
    rclcpp::Node * node, const std::string & service_name, const rclcpp::QoS & qos,
    const uint32_t timeout_ms, proton::BundleHandle * response_bundle, CallbackT callback)
  {
  ...
  std::string getServiceName() override { return std::string(client_->get_service_name()); }
