...
struct IService
{
  ...
  bool waitForFuture(proton::BundleHandle & handle)
  {
    ...
    // Wait for future
    if (response_future.wait_for(std::chrono::milliseconds(timeout_ms_)) ==
...
struct TypedService : public IService
{
    ...
    CallbackT callback)
  {
    ...
    srv = node->create_service<ServiceT>(
      service_name,
      [this, node, service_name, &request_bundle, &response_bundle,
      callback](const std::shared_ptr<RequestROS> request, std::shared_ptr<ResponseROS> response)
      {
        auto start_millis =
        ...
        if (waitForFuture(response_bundle)) {
          ...
          RCLCPP_ERROR(node->get_logger(), "Service %s timed out waiting for a response",
            service_name.c_str());
        }

        auto delta =
        std::chrono::duration_cast<std::chrono::milliseconds>(
          std::chrono::system_clock::now().time_since_epoch()).count() - start_millis;
    ...
    CallbackT callback)
  {
    ...
    srv = node->create_service<ServiceT>(
      service_name,
      [this, &request_bundle, callback](const std::shared_ptr<RequestROS> request,
      std::shared_ptr<ResponseROS> response)
      {
        RCL_UNUSED(response);
        // Convert ROS request to Bundle
        ServiceTypeAdapter<ServiceT>::convert_to_bundle(*request, request_bundle);
