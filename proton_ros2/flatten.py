...
def get_mapping_config(name: str, info: dict) -> dict:
    ...
    if info.get('subpath'):
        mapping_config.update({ProtonROS2Config.Mapping.ROS2_SUBPATH: info['subpath']})

    if info['array']:
        mapping_config.update({ProtonROS2Config.Mapping.ROS2_LENGTH: info['bounded']})
    elif info['bounded'] > 0:
        # String capacity
        pass
...
def get_package_config(package: str) -> dict:
    ...
    for message, info in messages.items():
        ...
        for k, v in info.items():
            if v.get('stamp'):
                message_config.update({ProtonROS2Config.Message.STAMP: v.get('stamp')})
                continue
    ...
    for service, info in services.items():
        ...
        for k, v in info['request'].items():
            service_config[ProtonROS2Config.Service.MAPPING][
                ProtonROS2Config.Service.REQUEST
            ].append(get_mapping_config(k, v))
